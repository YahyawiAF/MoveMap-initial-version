...
export const getCountyDetailsUrl = (selectedCounty) => {
  return selectedCounty
    ? `/explore/us/${prettyUrlEncode(
        selectedCounty[consts.dbFields.stateAbbreviation]
      )}/${prettyUrlEncode(selectedCounty[consts.dbFields.name])}`
    : null;
};
...
export const meetsCriteria = (countyDatum, routerQuery) => {
  ...
  for (const key of filterKeys) {
    ...
    if (valueFromUrl) {
      if (urlKey === "canabis") {
        if(valueFromUrl === "all") return true
        let dataLegalValues = LegalCanabisForm.filter(
          (lg) =>
            lg.Legality === valueFromUrl &&
            lg.State.toLocaleLowerCase() ===
              countyDatum["state"]?.toLocaleLowerCase()
        );
        if (dataLegalValues.length > 0) return true;
        else false;
        return;
      }
      ...
      switch (compareMode) {
        ...
        case consts.compareModes.multipleChoice:
          ...
          if (
            valuesFromUrl != consts.allOption.value &&
            !valuesFromUrl.includes(valueFromCsv)
          ) {
            return false;
          }
          break;
        case consts.compareModes.multipleChoiceMappedToValues:
          ...
          for (let index = 0; index < valuesFromUrl2.length; index++) {
            ...
            const thisOption = filterOptions.find(
              (option) => option.value === urlValue
            );
            if (urlValue === consts.allOption.value) {
              hasMatch = true;
            } else if (
              (!thisOption.minimumValue ||
                valueFromCsv > thisOption.minimumValue) &&
              (!thisOption.maximumValue ||
                valueFromCsv <= thisOption.maximumValue)
            ) {
              hasMatch = true;
            }
          }
        ...
        case consts.compareModes.singleChoice:
          const csvFilterValues =
            valueFromUrl &&
            filter.options[valueFromUrl] &&
            filter.options[valueFromUrl].csvValues;
          if (
            valueFromUrl &&
            valueFromUrl != consts.allOption.value &&
            !csvFilterValues.includes(valueFromCsv)
          ) {
            return false;
          }
          break;
        case consts.compareModes.boolean:
          ...
          if (
            valueFromUrl &&
            valueFromUrl == consts.defaultUrlTrueValue &&
            valueFromCsv != trueValue
          ) {
            return false;
          }
          break;
